      ...
      <v-card class="white pa-3">
        ...
        <v-form v-if="!loading" v-model="isFormValid">
          <v-row class="mt-5">
            ...
            <v-col v-for="(student, index) in studentsData" :key="student.account_id" cols="12" md="6"
...
export default {
  data() {
    return {
      ...
      xlsxData: {
        ...
        filename: `التاريخ (${this.$route.params.date})`,
        autoWidth: true,
        bookType: 'xlsx',
      },
  ...
  methods: {
    async getStudentData() {
      ...
      const response = await Api.getAbsenceAccountToAdd(study_year, this.$route.params.date)
    ...
    async handleDownload() {
      ...
      this.xlsxData.downloadLoading = true
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['اسم الاستاذ', 'الهاتف', 'الحالة']
        const filterVal = ['account_name', 'account_mobile', 'absence_type']
