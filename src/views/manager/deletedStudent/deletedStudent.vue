  ...
  <div class="team">
    <v-container class="indigo lighten-5 my-5">
      <v-card class="white pa-3">
        <h1 class="text-center subtitle-4 black--text"> الطلاب المحذوفين </h1>
        <v-row class="mt-5">
          <v-col md="3" xs="12">
          </v-col>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    async getTeacherDataAxios() {
      ...
      const response = await Api.getStudentsDeleted(page, itemsPerPage, search)
    ...
    async recoverConfirm() {
      ...
      const response = await Api.restoreDeletedStudent(this.deletedItem._id)
