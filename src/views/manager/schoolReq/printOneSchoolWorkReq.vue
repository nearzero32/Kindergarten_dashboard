    ...
    <div class="page">
      ...
      <div>
        ...
        <div>
          ...
          <h2>الكلية: {{ item.college }}</h2>
          <h2>اخر عمل: {{ item.last_work }}</h2>
          <h2>أقرب نقطة دالة: {{ item.nearest_landmark }}</h2>
          <h2>رقم الهاتف: {{ item.phone1 }}</h2>
          <h2>رقم الهاتف الثاني: {{ item.phone2 }}</h2>
          <h2>الحالة الأجتماعية: {{ item.social_status }}</h2>
          <h2>سنة التخرج: {{ item.year_graduate }}</h2>
        </div>
        <!-- Right Content -->
      </div>
    </div>
...
export default {
  ...
  async created() {
    ...
    this.item = JSON.parse(localStorage.getItem('printOneSchoolWorkReq'))
