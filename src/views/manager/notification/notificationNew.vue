          ...
          <v-col md="3" xs="12">
            <div class="d-md-flex">
              <v-btn tile color="primary" class="ml-2 mb-4 mb-md-0 mb-sm-0" :block="isScreenXs" @click="goToAddPage">
                اضافة اشعار <v-icon right style="font-size: 15px"> fa-plus </v-icon>
              </v-btn>
          ...
          <v-col cols="12">
            <v-data-table :headers="table.headers" loading-text="جاري التحميل ... الرجاء الانتظار"
              ...
              <template v-slot:item.receiver="{ item }">
                <!-- <span
                  style="color: #000"
                  @mouseover="showRecievers(item.receiverType)"
                >{{ showRecieverType(item.receiverType) }}</span> -->
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <span style="color: #000" v-bind="attrs" v-on="on" @mouseover="showRecievers(item.receiverType)">{{
                        showRecieverType(item.receiverType)
                    }}</span>
                  </template>
                ...
                <!-- <ul v-if="item.receiverType === 'class_school'">
                  <li
                    v-for="(user, index) in item.receiver"
                    :key="index"
                  >
                    {{ user.classes_name + "_" + user.class_school_leader }}
                  </li>
                </ul>
                <ul v-else>
                  <li
...
export default {
  ...
  methods: {
    ...
    goToAddPage() {
      this.$router.push('/notification/addNotification')
    },
